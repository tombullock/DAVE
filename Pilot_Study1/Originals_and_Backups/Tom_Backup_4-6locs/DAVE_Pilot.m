function DAVE_Pilot(sjNum,practice,session,nTrials,nBlocks,window,thisRect,window2,eyetracker,writeCodes,dual,loc_manipulation)
%% Trial log
trialLog = struct(...
    'experiment','',...
    'subNum',{},...
...
%                 distractColor = color6;
        ...
        if thisTargetType == 1
            targetLoc = [(LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(left_tilt_target(1,1)) (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(left_tilt_target(1,1));...
                (LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(left_tilt_target(1,2)) (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(left_tilt_target(1,2))]';
            cResp =2;
        else
            targetLoc = [(LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(right_tilt_target(1,1)) (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(right_tilt_target(1,1));...
                (LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(right_tilt_target(1,2)) (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(right_tilt_target(1,2))]';
            cResp =1;
        end
        ...
        if session ==2
            if diamonds == 1 %diamond among circles
                ...
                Screen('FrameOval',window, distractColor, LocationMatrix(3,1:4),4);
                Screen('FramePoly',window, targetColor, polyCoords,4); %target
                if loc_manipulation==1
                    Screen('FrameOval',window, color5, LocationMatrix(5,1:4),4);
                    Screen('FrameOval',window, color6, LocationMatrix(6,1:4),4); %crit distracter [NEED THIS TO SHOW]
                end
                shape = 1; %diamond
                ...
                Screen('FrameOval',window, targetColor, LocationMatrix(4,1:4),4); %target
                if loc_manipulation==1
