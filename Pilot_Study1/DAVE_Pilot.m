function DAVE_Pilot(sjNum,practice,session,nTrials,nBlocks,window,thisRect,window2,eyetracker,writeCodes,dual,loc_manipulation,testMode)
%% Trial log
clear trialLog
trialLog = struct(...
    'experiment','',...
...
trialLog(1).experiment = 'DAVE Pilot 1';
...
if loc_manipulation==1
    ...
    LocationMatrixOriginal = LocationMatrix;

elseif loc_manipulation==2

    % Stim locations (FOUR)
    loccenterX = round(centerX + circlerad * cosd(45:90:315))'; % list of x coords for four locs
    loccenterY = round(centerY + circlerad * sind(45:90:315))'; % list of y coords for four locs
...
lefttilt = [315 135];
righttilt = [45 225];
left_tilt_target = lefttilt; % edits from horizontal and vertical targets
right_tilt_target = righttilt;


%Create stim
%colorlist = [22 128 109; 199 40 154; 140 111 78; 115 115 115; 169 60 203; 166 97 100; 186 93 16; 63 129 45; 146 111 16; 122 122 0];
colorlist = [22 128 109; 199 40 154; 140 111 78; 169 60 203; 166 97 100; 186 93 16; 63 129 45; 146 111 16; 122 122 0];
...
if session == 2
    ...
    %     % DAVE edit
    %     rewardcondition = repmat([1 2]',20,1); % 1 = reward 2 = no reward
    %     targettype = repmat([1 2]',20,1); % 1 = slanted left 2 = slanted right
    %
    ...
    % DAVE EDIT to ensure all target and distractor loc combos represented
    % equally

    if loc_manipulation==1 % six locations
        
        % load predefined sequence
        load('TrialSeq_Full_6Locs.mat')

        % replicate trial sequence to make 100 trials per target loc
        trialSequence = repmat(TrialSeq_Full_6Locs,[5,1]);

    elseif loc_manipulation==2 % four locations

        % load predefined sequence
        load('TrialSeq_Full_4Locs.mat')

        % replicate trial sequence to make 100 trials per target loc
        trialSequence = repmat(TrialSeq_Full_4Locs,[8,1]);
    ...
    % need to divide into blocks [6 locs: 600 = 6x100, 4 locs: 384 = 6*64]
    if loc_manipulation==1
        cnt=0;
        for i=1:60:600
            cnt=cnt+1;
            trialSequenceBlocked(:,:,cnt) = trialSequence(i:i+59,:); 
        end
    elseif loc_manipulation==2
        cnt=0;
        for i=1:64:384
            cnt=cnt+1;
            trialSequenceBlocked(:,:,cnt) = trialSequence(i:i+63,:);
        end
    end
...
% % TOM ADDED
% if nTrials> length(trialSequence)
...
% DAVE EDIT 
nBlocks = size(trialSequenceBlocked,3);
...
%     trialSequence = trialSequence(newOrder,:);      % applies newOrder to trialSequence and spits out randomized trial sequence (column vector)
    ...
    % DAVE EDIT
    trialSequence = squeeze(trialSequenceBlocked(:,:,thisBlock));
    ...
    if writeCodes == 1
        sendTrigger(lj,100+thisBlock);
    end

    for trial=1:nTrials

        % the full location matrix needs reinstating at start of each trial 
        LocationMatrix = LocationMatrixOriginal;
...
%         thisRewardSize = trialSequence(trial,1);
        thisTargetType = trialSequence(trial,4); % DAVE bar tilted left or right
        thisRewardSize = trialSequence(trial,3); % DAVE salient color (white) on targ shape or dist


        if thisRewardSize == 1
            %probs = [1 1 1 1 0]; % original
            probs = [1 1 1 1 1]; % DAVE edit
        else
            %probs = [0 0 0 0 0];
            probs = [1 1 1 1 1];
        end
...
%         end
        ...
        if session == 2
            if  thisRewardSize==1
                color1 = highColor; %colorlist(1,:);
                color2 = colorlist(2,:);
                color3 = colorlist(3,:);
                color4 = colorlist(4,:);
                color5 = colorlist(5,:);
                color6 = colorlist(6,:); %highColor; [original]
                rewardcond = 1;
                targetColor = color1;%color6;
                distractColor = color2;
            elseif  thisRewardSize==2
                color1 = highColor; %colorlist(1,:);
                color2 = colorlist(2,:);
                color3 = colorlist(3,:);
...
%         LocationMatrix = LocationMatrix(newOrder,:);
        ...
        % set the target and distractor locations from the trial seq
        targetLocCode = trialSequence(trial,1);
        distractLocCode = trialSequence(trial,2);

        targetLocCoords = LocationMatrix(targetLocCode,:);
        distractLocCoords = LocationMatrix(distractLocCode,:);

        % remove targ and dist locations from the matrix of locations and
        % then shuffle the remaining 4 [6locs] or 2 [4locs] location matz
        LocationMatrix([targetLocCode,distractLocCode] ,:) = [];
        newOrder = randperm(size(LocationMatrix,1));
        LocationMatrix = LocationMatrix(newOrder,:);


        if thisTargetType == 1
            targetLoc = [(targetLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(left_tilt_target(1,1)) (targetLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(left_tilt_target(1,1));...
                (targetLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(left_tilt_target(1,2)) (targetLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(left_tilt_target(1,2))]';
            cResp =2;
        else
            targetLoc = [(targetLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(right_tilt_target(1,1)) (targetLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(right_tilt_target(1,1));...
                (targetLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(right_tilt_target(1,2)) (targetLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(right_tilt_target(1,2))]';
            cResp =1;
        end

        distracters = randi([1 2]);
        if distracters == 1 %three right two left [MAY NEED TO SWITCH THIS UP FOR THE 4 LOCS STUDY]
            ...
            distLoc3 = [(distractLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(righttilt(1,1)) ...
                (distractLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(righttilt(1,1));...
                (distractLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(righttilt(1,2))...
                (distractLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(righttilt(1,2))]';
            if loc_manipulation==1
                distLoc4 = [(LocationMatrix(3,1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,1)) ...
                ...
                distLoc5 = [(LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,1)) ...
                    (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(lefttilt(1,1));...
                    (LocationMatrix(4,1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,2))...
                    (LocationMatrix(4,2)+stimsize/2)+(stimsize/2-4)*sind(lefttilt(1,2))]';
            end
            ...
            distLoc2 = [(LocationMatrix(2,1)+stimsize/2)+(stimsize/2-4)*cosd(righttilt(1,1)) ...
                ...
                (LocationMatrix(2,1)+stimsize/2)+(stimsize/2-4)*cosd(righttilt(1,2))...
                (LocationMatrix(2,2)+stimsize/2)+(stimsize/2-4)*sind(righttilt(1,2))]';
            distLoc3 = [(distractLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,1)) ...
                (distractLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(lefttilt(1,1));...
                (distractLocCoords(1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,2))...
                (distractLocCoords(2)+stimsize/2)+(stimsize/2-4)*sind(lefttilt(1,2))]';
            if loc_manipulation==1
                distLoc4 = [(LocationMatrix(3,1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,1)) ...
                    (LocationMatrix(3,2)+stimsize/2)+(stimsize/2-4)*sind(lefttilt(1,1));...
                    (LocationMatrix(3,1)+stimsize/2)+(stimsize/2-4)*cosd(lefttilt(1,2))...
        ...
        Screen('DrawLines',window,targetLoc,2,bar_color);
        Screen('DrawLines',window,distLoc1,3,bar_color);
        Screen('DrawLines',window,distLoc2,3,bar_color);
        Screen('DrawLines',window,distLoc3,3,bar_color);
        if loc_manipulation==1
            Screen('DrawLines',window,distLoc4,3,bar_color);
            Screen('DrawLines',window,distLoc5,3,bar_color);
        end
        ...
        if session ==2
            if diamonds == 1 %diamond among circles
                %diamond coordinates
                xCoord = [targetLocCoords(1) + (stimsize/2), targetLocCoords(1) - diamcor, targetLocCoords(1) + (stimsize/2), targetLocCoords(1) + stimsize + diamcor]';
                yCoord = [targetLocCoords(2) + stimsize + diamcor, targetLocCoords(2) + (stimsize/2), targetLocCoords(2)- diamcor, targetLocCoords(2) + (stimsize/2)]';
                polyCoords = [xCoord yCoord];

                %Draw colored ovals
                Screen('FrameOval',window, color4, LocationMatrix(1,1:4),4);
                Screen('FrameOval',window, color3, LocationMatrix(2,1:4),4);
                Screen('FrameOval',window, distractColor, distractLocCoords(1:4),4);
                Screen('FramePoly',window, targetColor, polyCoords,4); %target
                if loc_manipulation==1
                    Screen('FrameOval',window, color5, LocationMatrix(3,1:4),4);
                    Screen('FrameOval',window, color6, LocationMatrix(4,1:4),4); %crit distracter [NEED THIS TO SHOW]
                end
                shape = 1; %diamond
                ...
                xCoord = [distractLocCoords(1) + (stimsize/2), distractLocCoords(1) - diamcor, distractLocCoords(1) + (stimsize/2), distractLocCoords(1) + stimsize + diamcor]';
                yCoord = [distractLocCoords(2) + stimsize + diamcor, distractLocCoords(2) + (stimsize/2), distractLocCoords(2)- diamcor, distractLocCoords(2) + (stimsize/2)]';
                ...
                shape = 2; %circle
                %Draw colored ovals
                Screen('FramePoly',window, color4, polyCoords1,4);
                Screen('FramePoly',window, color3, polyCoords2,4);
                Screen('FramePoly',window, distractColor, polyCoords3,4);
                Screen('FrameOval',window, targetColor, targetLocCoords(1:4),4); %target
                if loc_manipulation==1
                    Screen('FramePoly',window, color5, polyCoords5,4);
                    Screen('FramePoly',window, color6, polyCoords6,4); %crit distracter [NEED TO SHOW]
                end
            end
        end
...
%             Screen('FrameOval',window, color3, distractLocCoords(1:4),4);
%             Screen('FrameOval',window, color4, targetLocCoords(1:4),4); %target
%             Screen('FrameOval',window, color5, LocationMatrix(3,1:4),4);
%             Screen('FrameOval',window, color6, LocationMatrix(4,1:4),4); %crit distracter
%         end
        ...
        while 1
            ...
            [~, secs, keycodes] = KbCheck(keyboardIndices);
        ...
        if practice == 0 && session==2
            trialLog.trialInfo(trial+trialcount).n_locations_manipulation = loc_manipulation;
            trialLog.trialInfo(trial+trialcount).rewardcondition = rewardcond;
            trialLog.trialInfo(trial+trialcount).targettype = thisTargetType;
            trialLog.trialInfo(trial+trialcount).targetLocCode = targetLocCode;
            trialLog.trialInfo(trial+trialcount).distractLocCode = distractLocCode;
            trialLog.trialInfo(trial+trialcount).correctresp = cResp;
            trialLog.trialInfo(trial+trialcount).response = resp;
            trialLog.trialInfo(trial+trialcount).accuracy = Acc;
            trialLog.trialInfo(trial+trialcount).rt = reactionTime;
        ...
        elseif practice == 0 && session ==2
            save(sprintf('sj%02d_nLocs%02d_DAVE_PILOT_1', sjNum,loc_manipulation), 'trialLog');
        end
